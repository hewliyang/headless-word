#!/usr/bin/env python3
"""
Linux daemon helper for headless-word.

Runs with system Python (which has the uno module) and bridges
TCP commands to LibreOffice via UNO socket connection.

Usage: /usr/bin/python3 linux_helper.py

Env vars:
    HEADLESS_WORD_PORT          TCP port for the daemon
    HEADLESS_WORD_IDLE_TIMEOUT  Seconds before auto-exit
"""
...
DAEMON_PORT = int(os.environ.get("HEADLESS_WORD_PORT", str(PRIVATE_PORT_START)))
UNO_PORT = 2002
UNO_HOST = "127.0.0.1"
...
# ---------------------------------------------------------------------------
# Helpers (duplicated from macro.py since this runs in system Python)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Tool implementations (same logic as macro.py)
# ---------------------------------------------------------------------------
...
def _extract_body_content(docx_path, start_child=None, end_child=None):
        ...
        for sid in style_ids:
            ...
            if el:
                for based_on in el.getElementsByTagNameNS(W_NS, "basedOn"):
                    base = based_on.getAttributeNS(
                        W_NS, "val"
                    ) or based_on.getAttribute("w:val")
                    if base:
                        to_include.add(base)
        parts = []
        ...
        for num_el in num_dom.getElementsByTagNameNS(W_NS, "num"):
            ...
            if nid and nid in num_ids:
                ...
                for abs_ref in num_el.getElementsByTagNameNS(W_NS, "abstractNumId"):
                    aid = abs_ref.getAttributeNS(W_NS, "val") or abs_ref.getAttribute(
                        "w:val"
                    )
                    if aid:
                        abstract_ids.add(aid)
        for abs_el in num_dom.getElementsByTagNameNS(W_NS, "abstractNum"):
            aid = abs_el.getAttributeNS(W_NS, "abstractNumId") or abs_el.getAttribute(
                "w:abstractNumId"
            )
    ...
    for i, el in enumerate(all_elements):
        ...
        if tag == "tbl":
            ...
            if alias_els:
                title = (
                    alias_els[0].getAttributeNS(W_NS, "val")
                    or alias_els[0].getAttribute("w:val")
                    or ""
                )
...
def tool_get_paragraph_ooxml(doc, work_dir, params):
    start_idx = params["paragraph_index"]
    end_idx = params.get("end_paragraph_index", start_idx)
    if end_idx is None:
        end_idx = start_idx
    ...
    all_paras = body.getElementsByTagNameNS(W_NS, "p")

    if start_idx >= len(all_paras):
        raise ValueError(
            f"Paragraph index {start_idx} out of range (0-{len(all_paras) - 1})"
        )
    if end_idx >= len(all_paras):
        raise ValueError(
            f"End paragraph index {end_idx} out of range ({start_idx}-{len(all_paras) - 1})"
        )
    ...
    for i in range(start_idx, end_idx + 1):
        p = all_paras[i]
        clean_element(p)
        para_parts.append(p.toxml())
        for tag in ("pStyle", "rStyle"):
            for el in p.getElementsByTagNameNS(W_NS, tag):
                val = el.getAttributeNS(W_NS, "val") or el.getAttribute("w:val")
                if val:
                    style_ids.add(val)
        for el in p.getElementsByTagNameNS(W_NS, "numId"):
            val = el.getAttributeNS(W_NS, "val") or el.getAttribute("w:val")
            if val and val != "0":
                num_ids.add(val)
            ...
            if el:
                for based_on in el.getElementsByTagNameNS(W_NS, "basedOn"):
                    ...
                    if base:
                        to_include.add(base)
        for dd in styles_dom.getElementsByTagNameNS(W_NS, "docDefaults"):
            style_xml_parts.append(dd.toxml())
        for sid in to_include:
            el = style_map.get(sid)
            if el:
                clean_element(el)
                style_xml_parts.append(el.toxml())
    ...
    if raw_numbering and num_ids:
        ...
        for num_el in num_dom.getElementsByTagNameNS(W_NS, "num"):
            nid = num_el.getAttributeNS(W_NS, "numId") or num_el.getAttribute("w:numId")
            if nid and nid in num_ids:
                numbering_xml_parts.append(num_el.toxml())
                for abs_ref in num_el.getElementsByTagNameNS(W_NS, "abstractNumId"):
                    aid = abs_ref.getAttributeNS(W_NS, "val") or abs_ref.getAttribute(
                        "w:val"
                    )
        ...
        for abs_el in num_dom.getElementsByTagNameNS(W_NS, "abstractNum"):
            ...
            if aid and aid in abstract_ids:
                numbering_xml_parts.insert(0, abs_el.toxml())

    output_parts = []
    if style_xml_parts:
        output_parts.append("<!-- Referenced styles -->\n" + "\n".join(style_xml_parts))
    if numbering_xml_parts:
        output_parts.append(
            "<!-- Numbering definitions -->\n" + "\n".join(numbering_xml_parts)
        )
    range_label = (
        f"s {start_idx}-{end_idx}" if start_idx != end_idx else f" {start_idx}"
    )
    output_parts.append(f"<!-- Paragraph{range_label} -->\n" + "\n".join(para_parts))

    return {
        "paragraph_index": start_idx,
        "end_paragraph_index": end_idx if end_idx != start_idx else None,
        "xml": "\n\n".join(output_parts),
    }
...
def tool_insert_ooxml(doc, work_dir, params):
    ...
    if position == "start":
        ...
        if idx >= len(elements):
            raise ValueError(
                f"Body child index {idx} out of range (0-{len(elements) - 1})"
            )
...
TOOLS = {
    ...
    "get_paragraph_ooxml": tool_get_paragraph_ooxml,
...
def handle_command(cmd):
    global desktop, sessions
    try:
        if cmd == "PING":
            return "PONG"

        if cmd == "QUIT":
            for _sid, (doc, _fp, wd) in list(sessions.items()):
                with contextlib.suppress(Exception):
                    doc.close(False)
                with contextlib.suppress(Exception):
        ...
        if cmd == "LIST":
            info = {}
            for sid, (_doc, fp, _wd) in sessions.items():
                info[sid] = {"path": fp}
        ...
        if cmd.startswith("TOOL:"):
            ...
            params = (
                json.loads(base64.b64decode(params_b64).decode()) if params_b64 else {}
            )
    ...
    while True:
        try:
            ...
            if time.time() - last_activity > IDLE_TIMEOUT:
                for _sid, (doc, _, wd) in list(sessions.items()):
                    with contextlib.suppress(Exception):
                        doc.close(False)
                    with contextlib.suppress(Exception):
                        shutil.rmtree(wd, ignore_errors=True)
